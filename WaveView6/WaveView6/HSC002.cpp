...
HSC::OpenImageFromFile(std::string filename)
{
  ...
  m_image.display("before blur", true);

  //
  double diffsion = 2.0;
  m_image.blur(diffsion);
  m_image.display("after blur", true);
...
HSC::FindGridPicth()
{
  ...
  m_resultImage(m_centerX, m_centerY, 0, 0) = CENTROID_FLAG;
        ...
        if( maxVal > m_CENTROID_THRESHOLD )
        {
          ...
          float avgX =  (x*sum3x3  - (m_image(x-1, y-1) + m_image(x-1, y)+m_image(x-1, y+1))
           ...
           float avgY = (y*sum3x3  - (m_image(x-1, y-1) + m_image(x, y-1)+m_image(x+1, y-1)) 
            ...
            m_resultImage(localMaxX, localMaxY, 0, 0) = CENTROID_FLAG;
...
bool HSC::LocateMissingCentroid()
{
  ...
  float minMaxDist = abs((int)closestToCenterX - imgCenterX) + abs((int)closestToCenterY - imgCenterY);
