...
//////////////////////////////////////////////////////////////////////////
const double  m_K[ZERNIKE_MAX_ORDER+1][ZERNIKE_MAX_ORDER+1] =
{	
...
public:
	...
	double ZernikeEvalue(double x, double y);
	...
	void Excute();
	bool Test();
	void TestZernikeTerms();
	bool CalcZernike(	std::vector< LLWPoint2D<float> > dPoints,
...
private:
	...
	double ZernikePoly(unsigned int k, double x, double y);
	double ZernikePoly_Dx(unsigned int k, double x, double y);
	double ZernikePoly_Dy(unsigned int k, double x, double y);
	...
	bool MapKtoIJ(const int k, int& i, int& j);

private:	
	double m_dCoefficient[ZERNIKE_MAX_ORDER+1][ZERNIKE_MAX_ORDER+1];
	double** m_ppMatrix;
	double* m_pB;
	double* m_pX;
	double* m_pRefX;
	...
	int m_nOrder;
	double m_dFocus;	// foucs length
	double m_dScalor; // normalization factor in pixel
	double m_dAnalysisZoneRadius;
	LLWPoint2D<float> m_center;

	CImg<float> m_coMatrix;
	CImg<float> m_zernikeCoe;
	CImg<float> m_nonHomeCoe;
